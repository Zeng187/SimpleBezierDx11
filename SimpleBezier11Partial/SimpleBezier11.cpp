...
PartialCase3 m_PartialCase3;
PartialCase4 m_PartialCase4;
    ...
    void* pUserContext)
{
    ...
    V_RETURN(m_PartialCase0.CreateShaders(pd3dDevice));
    V_RETURN(m_PartialCase1.CreateShaders(pd3dDevice));
    V_RETURN(m_PartialCase2.CreateShaders(pd3dDevice));
    V_RETURN(m_PartialCase3.CreateShaders(pd3dDevice));
    V_RETURN(m_PartialCase4.CreateShaders(pd3dDevice));
    ...
    //D3D_SHADER_MACRO integerPartitioning[] = { { "BEZIER_HS_PARTITION", "\"integer\"" }, { "TRI_DOMAIN", "1"},{ "Case00", "1" }, { 0 } };
    //D3D_SHADER_MACRO fracEvenPartitioning[] = { { "BEZIER_HS_PARTITION", "\"fractional_even\"" }, { "TRI_DOMAIN", "1"},{ "Case00", "1" }, { 0 } };
    //D3D_SHADER_MACRO fracOddPartitioning[] = { { "BEZIER_HS_PARTITION", "\"fractional_odd\"" }, { "TRI_DOMAIN", "1"},{ "Case00", "1" }, { 0 } };

    D3D_SHADER_MACRO PartialCaes[] = { { "BEZIER_HS_PARTITION", "\"integer\"" },{ "TRI_DOMAIN", "1"} ,{ "CASE00", "1" } ,{0} };

    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", NULL, "BezierVS", "vs_5_0", &pBlobVS));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", PartialCaes, "BezierHS", "hs_5_0", &pBlobHSInt));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", PartialCaes, "BezierHS", "hs_5_0", &pBlobHSFracEven));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", PartialCaes, "BezierHS", "hs_5_0", &pBlobHSFracOdd));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", PartialCaes, "BezierDS", "ds_5_0", &pBlobDS));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", NULL, "BezierPS", "ps_5_0", &pBlobPS));
    V_RETURN(CompileShaderFromFile(L"SimpleBezierPartial.hlsl", NULL, "SolidColorPS", "ps_5_0", &pBlobPSSolid));
    ...
    float fElapsedTime, void* pUserContext)
{
    ...
    //m_PartialCase0.DrawIndexedRegular(pd3dImmediateContext, cvs_cnt);
    //m_PartialCase1.DrawIndexedRegular(pd3dImmediateContext, cvs_cnt);
    //m_PartialCase2.DrawIndexedRegular(pd3dImmediateContext, cvs_cnt);
    //m_PartialCase3.DrawIndexedRegular(pd3dImmediateContext, cvs_cnt);
    m_PartialCase4.DrawIndexedRegular(pd3dImmediateContext, cvs_cnt);

    // Draw the mesh
    //pd3dImmediateContext->Draw(cvs_cnt, 0);
...
void CALLBACK OnD3D11DestroyDevice(void* pUserContext)
{
    ...
    SAFE_DELETE(geom_data);

    m_PartialCase0.DestroyShaders();
    m_PartialCase1.DestroyShaders();
    m_PartialCase2.DestroyShaders();
    m_PartialCase3.DestroyShaders();
    m_PartialCase4.DestroyShaders();
}

bool ReadGeomData()
{
    std::ifstream geom_file;
    geom_file.open("data/geom.bin", std::ios::binary);
    if (!geom_file.is_open())
    {
        std::cout << "fail to open Bezier geom file" << std::endl;
        return false;
    }

    //geom_file.read(reinterpret_cast<char*>(&reg_cnt), sizeof(int));
    //vector<unsigned char> geom_buffer(std::istreambuf_iterator<char>(geom_file), {});
    //geom_file.close();
    //cvs_cnt = 16 * reg_cnt;

    std::vector<unsigned char> geom_buffer(std::istreambuf_iterator<char>(geom_file), {});
    cvs_cnt = geom_buffer.size() / sizeof(float3);
    patch_cnt = cvs_cnt / 16;;

    patch_cnt = 12;
    cvs_cnt = patch_cnt * 16;
    geom_data = new float3[cvs_cnt];
    memcpy(geom_data, geom_buffer.data(), sizeof(float3) * cvs_cnt);
