...
// Control variables
float                               g_fSubdivs = 32;          // Startup subdivisions per side
...
void InitApp()
{
    ...
    D3DXVECTOR3 model_size(2.0f, 2.0f, 2.0f);
    D3DXVECTOR3 model_center(0.3f, 0.2f, 0.5f);
    float radius = 2 * model_center.z;
    radius = max(0.05f, radius);
    D3DXVECTOR3 front(0.0f, 0.0f, 1.0f);
    D3DXVECTOR3 vecEye = model_center - front * radius;

    D3DXVECTOR3 vecAt = model_center;
    g_Camera.SetViewParams( &vecEye, &vecAt );
    
    float diag_len = D3DXVec3Length(&model_size);
    float fNear = diag_len * 0.01f;
    float fFar= diag_len * 10.0f;
    float fAspectRatio = DXUTGetWindowWidth() / (FLOAT)DXUTGetWindowHeight();
    g_Camera.SetProjParams(D3DX_PI / 4, fAspectRatio, fNear, fFar);
...
bool ReadGeomData()
{
    ...
    //patch_cnt = 1;
    //cvs_cnt = 16 * patch_cnt;

    //const int MUL_TIMES = 2; // CREATE PARENT NODE
    //geom_data = new float3[ MUL_TIMES * cvs_cnt];
    //float3* geom_buffer_data = (float3*)(geom_buffer.data());
    //for (int k = 0; k < patch_cnt; k++)
    //{
    //    for (int v = 0; v < 16; v++)
    //    {
    //        for (int c = 0; c < MUL_TIMES; c++)
    //        {
    //            geom_data[MUL_TIMES * 16 * k + 16 * c + v] = geom_buffer_data[k * 16 + v];
    //        }
    //    }
    //}
    //cvs_cnt = cvs_cnt * MUL_TIMES;
